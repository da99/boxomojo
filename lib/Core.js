...
var Core = {
  'Vars' : {
    ...
    'Functions'        : {
      ...
      'is/update:' : function (box) {
        box.ensure_Returns_not_empty();
        if( box.is_var(box.grab_forward()) )
          box.Tokens.unshift(Parse.To_Function_Call('update:'));
        else
          box.Tokens.unshift(Parse.To_Function_Call('is:'));
      ...
      '][' : function (box) {
        ...
        if (!target.Tokens)
          box.throw_function_call_error('wrong kind of object: ' + box.kind);
        if (!list.Returns)
          box.throw_function_call_error('wrong kind of object: ' + list.kind);
      ...
      '<+[' : function (box) {
        ...
        if (old_func)
          throw new Error(box['function call'] + ': function already exists: ' + name);
