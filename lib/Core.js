...
var Core = {
  ...
  'run': function (box) {
    ...
    var new_box = box.copy(fs_func.Tokens);
    new_box.Vars = box.Vars;
    new_box.run();
    if (new_box.Returns.length > 0)
      box.respond(new_box.see_backward());
    return true;
  },
  ...
  //       typed cast to Boxomojo strings
  //       when reading keys.
  // ==============================================================
  ...
  '~{' : function (box) {
    var returns_reqs = box.grab_forward();
    var tokens_reqs  = box.grab_forward();
    var fin_reqs     = box.grab_forward();
    var func         = box.grab_forward();
    var end          = box.grab_forward(true);
    if (!end || end.value != '}~') {
      box.throw_function_call_error('closing must be: }~ , not: ' + end.value);
    };

    return true;
  },
  '~{' : function (box) {
    var ret = box.grab_forward(),
    tok     = box.grab_forward(),
    fin     = box.grab_forward(),
    fsfunc  = box.grab_forward(),
    end     = box.grab_raw();

    if (!ret || !tok || !fin || !fsfunc || !end ) {
      throw new Error('Missing values.');
    };

    if ( end.value !== '}~' ) {
      box.throw_function_call_error('expecting: }~ , not: ' + end.value);
    };
