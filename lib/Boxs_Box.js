...
h = require('boxomojo/lib/helpers'),
Func_Finder = require('boxomojo/lib/Func_Finder'),
...
o.fs = {
  ...
  'x': function (box) {
    ...
    return box.target.Vars[box.grab_forward()];
  },
...
o.js = {
  ...
  respond : function (val) {
    if (arguments.length == 0)
      throw new Error('boxomojo error: No value was given.' );
    if (arguments.length != 1)
      throw new Error('boxomojo error: Only one value is allowed'  + arguments.length.toString() );
    return this.Returns.push(val);
  },
  ...
  compile_forward : function () {
    ...
    if( fin === null || fin === undefined )
      throw new Error('Unknown Boxomojo raw type: ' + raw);
  ...
  is_applicable_func : function (finder) {

    var func = finder.value;
    ...
    if (this.Returns.length >= back.names && this.Tokens.length >= forw.names.length )
      return true

    this['[?!]'].unshift({ '{}': finder.func_name, '?!': "Lengths do not match."});
    return false;
  },
  ...
  run_func_if_found : function (finder) {
    ...
    while( finder.next() ) {
      if (this.is_applicable_func(finder)) {
  ...
  run: function (times) {
    ...
    if (_.last(this.Returns) != env) {
      this.Object = null;
      var finder = new Func_Finder.Box(env, this[':)~']);
    ...
    if(out && env != out) {
      finder = new Func_Finder.Box( out, this[':)~'] );
      if (this.run_func_if_found(finder))
        return this.run(times);
    };
    ...
    throw({
      'name': 'Error',
      'message': 'Function not found: ' + this[':)~'],
      '[?!]': this['[?!]']
    });
  } // === run
