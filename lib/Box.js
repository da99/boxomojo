...
Box.prototype.compile_forward = function () {
  ...
  var raw = this.Tokens.pop(),
      fin = null;

  if (raw['function call?']) {
    throw new Error('handle function call');

  } else if (raw['run now function?']) {
    ...
    throw new Error('handle list');

  } else if (raw['kv list?']) {
    throw new Error('handle kv list');

  } else
    throw new Error('Unknown Factor_Script raw type: ' + raw);

  this.Tokens.unshift(fin);
};
