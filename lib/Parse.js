...
var Grabs = 'grab_number grab_block grab_delimiter grab_func_call'.split(' ');
var Parse = function Parse (m) {
  this.raw = m.Code.split(/(\s+)/);
  return Parse.grab(raw);
};

Parse.grab = function (raw_arr) {
  ...
  while (raw_arr.length != 0) {
    ...
    while(++grab_i < grab_len) {
      ...
      while ( raw_arr.length != 0  ) {
        ...
        len    = raw_arr.length;
        insert = Parse[Grabs[grab_i]](raw_arr);
...
Parse.grab_number = function(raw_arr) {
  var token = raw_arr[0].trim();

  if (token.trim() === '')
    return null;

  token = Number(token);
  if (!_.isNumber(token) || _.isNaN(token)) {
    return null;
  };
...
Parse.grab_func_call(raw_arr) {
  var token = raw_arr[0].trim();
  if (token.trim() === '')
    return null;
  raw_arr.shift();
  return Parse.To_Func_Call(token);
};
