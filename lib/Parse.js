...
var Grabs = 'grab_block grab_delimiter grab_number grab_func_call'.split(' ');
...
Parser.prototype.grab_delimiter = function () {
  ...
  for ( start in Delimiters ) {
    ...
    if (_.first(parser.trim) !== start) {
      return null;
    };

    if (_.last(parser.trim) === end) {
      str = parser.shift();
      break;
    };
