...
Parser.prototype.loop_and_grab = function () {
...
Parser.prototype.grab_block = function() {
  ...
  var to_raw_name = b[1];
  if (to_raw_name == 'To_Comment') {
    this.loop_and_grab();
    return null;
  } else {
    var to_raw_func = Parse[to_raw_name];
...
Parse.To_Function_Call = function (name) {
  var r = Parse.To_Raw('function call', [name]);
  r.value = name;
  return r;
};

Parse.To_Run_Now_Function = function (arr) {
  return Parse.To_Raw('run now function', arr); 
};

Parse.To_Function = function (arr) {
  return Parse.To_Raw('function', arr);
};

Parse.To_List = function (arr) {
  return Parse.To_Raw('list', arr);
};

Parse.To_Index = function (arr) {
  return Parse.To_Raw('index', arr);
};

Parse.To_Object = function (arr) {
  return Parse.To_Raw('object', arr);
};
