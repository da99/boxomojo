...
B.prototype.find_and_run = function (name, box) {
...
B.prototype.front = function (val) {
  ...
  if (_.isObject(v) && v['func_call?']) {
    ...
    var results = me.find_and_run(v.value, box);
    if (!results)
      me.throw('Not defined: ' + v.value);
  ...
  vals: {
    '+' : [ function (box) {
      ...
      if ( ! (IS_LIST(a) && IS_LIST(b)) )
        return false;
      var new_box = Box.new(' [ ] ');
      new_box.Returns = a.Returns.concat(b.Returns);
      box.front( new_box );
      return true;
    }],
    '='  : function (box) {
