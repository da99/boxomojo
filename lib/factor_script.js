...
Box.prototype.env = function () {
  var inside = this.Core_Vars['[<>]'],
  out = this.Core_Vars['^[]^'];
...
Box.prototype.read_write_Outside = function () {
  this.read_in = false;
  if (!this.Core_Vars['^[]^'])
    this.throw_function_call_error('Programmer error: No Outside.');
  this.Core_Vars['[<>]'] = this.Core_Vars['^[]^'];
  return true;
};
...
Box.prototype.write_var = function (name, val) {
  if (this.write_in)
    this['Vars'][name] = val;
  else
...
Box.prototype.run = function () {
  ...
  if (_.last(this.Returns) != this.env() && this.read_in) {
    ...
    var finder = new Func_Finder.Box(this.Object, this['function call']);
  ...
  if (!this.read_in) {
    finder = new Func_Finder.Box( this.env(), this['function call'] );
    if (this.run_func_if_found(finder))
      return this.run();
  };
  ...
  throw new Error('Function not found: ' + this['function call']);
};
