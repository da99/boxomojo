...
function to_string(orig_tokens) {
  var tokens = orig_tokens.slice(),
  arr = [],
  temp = null;
  while (tokens.length !== 0)  {
    temp = tokens.shift();
    if (!temp || (temp.Tokens == undefined && temp.tokens == undefined) )
      arr.push(temp);
    else
      arr = arr.concat(to_string(temp.Tokens || temp.tokens));
  };
  ...
  while( finder.next() ) {
    ...
    if (this.is_applicable_func(finder.value)) {
      ...
      if (_.isEqual( _.uniq(check_box.Returns), [true] ) )
        return true;
      debugger;
      this.throw_function_call_error('returned values do not match requirements: ' + to_string(vals) + ' => ' + to_string(val.returns.Tokens) );
    };
