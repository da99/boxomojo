...
script.New.prototype.parse = function () {
  ...
  while( temp = raw_arr.shift() ) {
    ...
    start_block = script.BLOCK_ENDS[trim];
    if (start_block) {
      if ( !in_block ) 
        throw new Error("Ending unopened block: " + trim);
      expected_end = trim;
      if ( last_block.char != start_block ) 
        throw new Error("Closing the wrong block. actual: " + trim + " expected: " + script.BLOCKS[last_block.char] );
